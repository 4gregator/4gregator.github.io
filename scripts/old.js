...
function closeCombat() {
	movePts--;
	if (totalGunsOpp.valueOf() <= totalGuns.valueOf() && evasionOpp > 0) {
		evasionOpp--;
		alert("Компьютер использует уклонение. Уклонений осталось: " + evasionOpp);
		makeAction(); //не указаны параметры функции, хз как их сюда теперь всунуть
	} else combatResult();
}
